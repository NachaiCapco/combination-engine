...
from app.services.vpms.vpms_auth_service import get_auth_header
import os

# VPMS-specific hardcoded configuration
VPMS_STORAGE_PATH = Path(os.getcwd()) / "workspace"

def safe_name(name: str) -> str:
    return re.sub(r"[^A-Za-z0-9_\-]+", "_", name).strip("_") or "TestForgeSuite"

def setup_workspace(test_name: str) -> Tuple[Path, Path, Path]:
    root = VPMS_STORAGE_PATH / safe_name(test_name)
    gen = root / "generated"
    rep = root / "Report"
...
def generate_robot_cases_from_excel(excel_path: Path, gen_dir: Path):
    ...
    for i, row in df.iterrows():
        ...
        # Add VPMS-specific hardcoded headers
        # These headers are required for Prudential VPMS API requests
        # TODO: Add your VPMS headers here before running
        vpms_headers = {
        ...
        # Apply VPMS headers only if not already specified by user
        for key, value in vpms_headers.items():
            if key not in headers:
                headers[key] = value

        # Add VPMS authentication Bearer token to headers
        # Fetch Bearer token from VPMS auth API and add to Authorization header
        try:
            auth_header = get_auth_header()
            headers.update(auth_header)
        except Exception as e:
            # Log error but continue - auth might not be required for all endpoints
            print(f"Warning: Failed to get VPMS auth token: {e}")
